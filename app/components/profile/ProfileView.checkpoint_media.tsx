...
const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  // Debug logging
  useEffect(() => {
    console.log('🔍 ProfileView rendering with:', {
      ...
      spotlightItemsData: spotlightItems,
      shopItemsData: shopItems
    });
    
    // More detailed logging
    if (mediaItems.length > 0) {
      console.log('Media Items Present:', {
        firstItem: mediaItems[0],
        hasEmbedUrl: !!mediaItems[0].embedUrl,
        type: mediaItems[0].type
      });
    }
    
    if (spotlightItems.length > 0) {
      console.log('Spotlight Items Present:', {
        firstItem: spotlightItems[0],
        hasImage: !!spotlightItems[0].image
      });
    }
    
    if (shopItems.length > 0) {
      console.log('Shop Items Present:', {
        firstItem: shopItems[0],
        hasImage: !!shopItems[0].image
      });
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="container mx-auto p-4 sm:p-8 md:p-12 lg:p-16">
          ...
          <div className="bg-gray-800 text-white p-4 rounded-lg mb-4">
            <h3 className="font-bold">Sticker Debug (View)</h3>
            <p>profile.sticker: {JSON.stringify(profile.sticker)}</p>
            <p>Sticker visible: {profile.sticker?.visible ? 'Yes' : 'No'}</p>
            <p>Sticker image: {profile.sticker?.image?.substring(0, 30)}...</p>
          </div>
          <StickerSection
